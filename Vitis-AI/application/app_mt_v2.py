...
def runDPU(id,start,dpu,img, my_time):
    '''get tensor'''
    ...
    while count < n_of_images:
        ...
        dpu.wait(job_id)
        time2 = time.time()
        out_q[count] = np.argmax(outputData[count][0][0])
...
def app(image_dir,csv_zip_dir,use_csv,threads,model):
    ...
    for i in range(threads):
        ...
        in_q = img[start:end]
        t1 = threading.Thread(target=runDPU, args=(i,start,all_dpu_runners[i], in_q, my_time))
        threadAll.append(t1)
        start=end
    ...
    timetotal_v2 =  np.sum(my_time)
    print("timetotal %4f , time_total_v2 %.4f" %(timetotal, timetotal_v2)) 
    fps = float(runTotal / timetotal_v2)
    print (divider)
    print("Throughput=%.2f fps, total frames = %.0f, time=%.4f seconds" %(fps, runTotal, timetotal_v2))
